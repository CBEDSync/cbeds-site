...
- **Clearing Netlify is safe; deleting a staged row is not.** What has already been
  collected is remembered in `Review: Netlify id` inside `draft/Submissions.xlsx`, so
  emptying the Netlify list changes nothing there and nothing comes back. But delete a
  row *by hand* while it is still in Netlify and the next run re-adds it — and once
  Netlify is cleared, `draft/Submissions.xlsx` is the only copy left.
